...
```{r}
simplify_fixed_values(validator(if (z >= 0 && z <= 0) y==0, z==0))
```
Dit ga ik toevoegen, maar daarvoor moet ik het omzetten van regels naar het MIP probleem aanpassen. (volgende week of zo).
**Enhancement**
...
Ah, dit is een regelrechte **bug**: hier ga ik naar kijken.
...
```{r}
detect_redundancy(validator(if (z >= 0 && z <= 0) z==10))
```
